...
description: Maintain and extend the dockndevai MCP server suite (mcp-keycloak, mcp-kubernetes, mcp-oci, mcp-kafka, mcp-clickhouse, mcp-debezium, mcp-azure, mcp-azure-devops, mcp-percona-pg, ossian-mcp, mcp-grafana, mcp-macos, mcp-outlook). Use when adding or changing a tool, touching the security/policy layer, bumping dependencies, cutting a release, or publishing to npm or an MCP registry — so the safe-by-default invariants and conventions are preserved. ossian-mcp is a thin RAG/memory adapter (npm name `ossian-mcp`, unscoped; gate flag `OSSIAN_ALLOW_FORGET`). mcp-grafana is a Grafana HTTP adapter (gate flag `GRAFANA_ALLOW_DELETE`; folder/datasource allowlists; ships a second skill `grafana-dashboards-and-queries` that stores dashboard/query standards). mcp-outlook is a Microsoft Graph mail adapter (browser sign-in via OAuth authorization-code + PKCE in `src/graph/auth.ts`; gate flags `OUTLOOK_ALLOW_SEND` for send/reply/forward and `OUTLOOK_ALLOW_DELETE` for deletes; folder allowlist/protected folders). All follow the same architecture and flows.
---
